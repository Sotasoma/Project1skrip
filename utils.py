...
def undistort(img, cal_dir='calibration_pickle.p'):
...
def draw_lanes(img, left_fit, right_fit, frameWidth,frameHeight,src):
    ...
    middle_x = (left_fit + right_fit)/2
    middle_pts = np.transpose(np.vstack((middle_x, ploty))).astype(np.int32)

    cv2.polylines(color_img, np.int32([middle_pts]), False, (0, 0, 255), thickness=10)
    cv2.polylines(color_img, np.int32([left]), False, (0, 0, 255), thickness=15)
    cv2.polylines(color_img, np.int32([right]), False, (0, 0, 255), thickness=15)
    cv2.arrowedLine(color_img, (640, 719), (640,690), (255,0,255), 5, tipLength=0.25)
